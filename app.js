...
function updateTimers(){
  ...
  if(showTarget.time){
    ...
    document.getElementById('t-name').textContent=titlePrefix+showTarget.name;
...
function renderQuizPath(p){
  ...
  // Build day states
  const DS=[];
  // Find earliest incomplete day (for "start here" indicator)
  ...
  for(let d=1;d<=TOTAL;d++){
    ...
    if(d<=todayDay && !(lg2&&lg2.quizDone)){earliestIncomplete=d;break;}
  }
  for(let d=1;d<=TOTAL;d++){
    ...
    let state='locked';
    if(d<todayDay)state=(lg&&lg.quizDone)?'done':'catchup';
    else if(d===todayDay)state='today';
    // Mark earliest incomplete day as 'next' (priority target)
    if(d===earliestIncomplete && d<todayDay)state='next';
    DS.push({day:d,state,dateISO:dk,isEarliest:(d===earliestIncomplete)});
  }
  ...
  const hasNextDay=earliestIncomplete>0&&earliestIncomplete<todayDay;
  const hintText=todayQuizDone&&earliestIncomplete<0
    ?'✅ أجبت على كل الأيام! 🏆 رائع!'
    :hasNextDay
      ?`👆 لديك ${todayDay-earliestIncomplete>1?'أيام':'يوم'} لم تجب عليها — ابدأ من اليوم ${earliestIncomplete}!`
      :todayQuizDone
        ?'✅ أجبت على أسئلة اليوم! 🌟'
        :'⭐ اضغط على محطتك لتبدأ مسابقة اليوم!';
  let html=`
  <style>
    ...
    .explorer-bubble{position:absolute;top:-20px;left:50%;transform:translateX(-50%);white-space:nowrap;font-size:1.35rem;animation:bounce-exp 1s ease-in-out infinite}
  ...
  <div class="map-header">
    <div class="map-header-top">
      ...
      <div style="flex:1;min-width:0">
        <div class="map-header-title">🧠 مسابقة رمضان داخل الخريطة</div>
        <div class="map-header-sub">رحلة فخمة — كل يوم 10 أسئلة: 4 سهلة • 3 متوسطة • 3 صعبة</div>
      </div>
  ...
  for(let row=0;row<totalRows;row++){
    ...
    display.forEach((ds,i)=>{
      ...
      html+=`<div class="map-node-wrap" style="${isMilestone?'margin-top:4px':''}">
        ...
        ${isToday||isNext?`<div class="explorer-bubble">${isNext?('👆'+(p.avatar||'🧒')):(p.avatar||'🧒')}</div>`:''}
  ...
  html+=`<div class="map-legend">
    <div class="map-legend-item"><div class="map-legend-dot" style="background:#27ae60"></div>أكمل ✅</div>
    <div class="map-legend-item"><div class="map-legend-dot" style="background:#ff6b35"></div>ابدأ هنا ▶️</div>
    <div class="map-legend-item"><div class="map-legend-dot" style="background:#b8860b;border:2px dashed #b8860b"></div>لحّق 📖</div>
...
function wireButtons(){
  ...
  // (تم نقل إدخال رمز العائلة إلى صفحة تسجيل الطفل)
  ...
  document.getElementById('btn-s0').addEventListener('click',()=>{
    ...
    const fam=(document.getElementById('s-famcode')?.value||'').trim().toUpperCase();
  ...
  document.getElementById('btn-save-quiz-time')?.addEventListener('click',()=>{});
