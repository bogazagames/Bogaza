...
// Journey progression tuned to feel منطقي عبر شهر رمضان (ليس سريعاً جداً)
const JOURNEY = [
  {id:'start',   name:'البداية',      icon:'🏡', req:0},
  {id:'village', name:'القرية',       icon:'🌳', req:350},
  {id:'desert',  name:'الصحراء',     icon:'🏜️', req:850},
  {id:'mountain',name:'الجبل',       icon:'⛰️', req:1450},
  {id:'mosque',  name:'المسجد',      icon:'🕌', req:2150},
  {id:'stars',   name:'النجوم',      icon:'⭐', req:2900},
  {id:'moon',    name:'القمر',       icon:'🌙', req:3700}
];
const ACHIEVEMENTS = [
...
function renderHome(){
  ...
  const quizUnlocked=fajrUnlockTime?nowH>=fajrUnlockTime:nowH.getHours()>=4;
...
function renderHomeInventory(p){
  ...
  const gifts=(p.giftHistory||[]).slice(0,8);
  ...
  const cards=inv.map(id=>itemsById.get(id)).filter(Boolean);
  const giftCards=gifts.map(g=>`
    <div class="inv-card" style="border-color:rgba(240,180,41,.35)">
      <div class="inv-ic">🎁</div>
      <div class="inv-n">${escapeHtml(g.text||'هدية')}</div>
    </div>`);
  strip.innerHTML=[...giftCards, ...cards.map(it=>`
    <div class="inv-card">
  ...
  list.querySelectorAll('[data-gtime]').forEach(btn=>{
    btn.addEventListener('click',async()=>{
      ...
      try{
        ...
        // Keep a visible place for gifts in child page (مقتنياتي)
        if(!p.giftHistory) p.giftHistory=[];
        p.giftHistory.unshift({
          time: Date.now(),
          text: (rw.text||rw.desc||'').toString(),
...
function updateTimers(){
  ...
  const todayTarget=getTarget(p,td);
  const fajrT=parseT(td.fajr,td.dateISO);
  const maghribT=parseT(td.maghrib,td.dateISO);

  // Determine next goal
  let goal={name:todayTarget.name,time:todayTarget.time,dateISO:td.dateISO,labelPrefix:'الهدف القادم: '};

  if(log.maghribReached){
    const tmr=new Date(td.dateISO+'T00:00:00');tmr.setDate(tmr.getDate()+1);
    const tmrISO=tmr.toISOString().split('T')[0];
    const td2=STATE.timetable.find(d=>d.dateISO===tmrISO) || td;
    const tmrTarget=getTarget(p,td2);
    goal={name:tmrTarget.name,time:tmrTarget.time,dateISO:td2.dateISO,labelPrefix:'هدف الغد: '};
  } else if(log.targetReached && !log.maghribReached){
    goal={name:'المغرب',time:td.maghrib,dateISO:td.dateISO,labelPrefix:'الهدف القادم: '};
  } else {
    const tt=parseT(todayTarget.time,td.dateISO);
    if(now>maghribT){
      const tmr=new Date(td.dateISO+'T00:00:00');tmr.setDate(tmr.getDate()+1);
      const tmrISO=tmr.toISOString().split('T')[0];
      const td2=STATE.timetable.find(d=>d.dateISO===tmrISO) || td;
      const tmrTarget=getTarget(p,td2);
      goal={name:tmrTarget.name,time:tmrTarget.time,dateISO:td2.dateISO,labelPrefix:'هدف الغد: '};
    } else if(now>tt && !log.targetReached){
      goal={name:todayTarget.name,time:todayTarget.time,dateISO:td.dateISO,labelPrefix:'الهدف الحالي: '};
    }
  }

  if(goal.time){
    const gt=parseT(goal.time,goal.dateISO);
    const diff=gt-now;
    document.getElementById('t-name').textContent=goal.labelPrefix+goal.name;
    document.getElementById('t-main').textContent=fmtTime(diff);
    const mins=diff/60000;
    document.getElementById('t-sub').textContent=diff<=0?'⏳ حان الوقت!':mins<=5?'🔥 بقي دقائق! أنت تقدر!':mins<=30?'⭐ قريب من الهدف!':'💪 استمر!';
    const pct=Math.min(100,Math.max(0,(now-fajrT)/(maghribT-fajrT)*100));
    document.getElementById('p-fill').style.width=pct+'%';
    const markerT=parseT(goal.time,goal.dateISO);
    const mPct=(markerT-fajrT)/(maghribT-fajrT)*100;
    document.getElementById('p-marker').style.right=(100-Math.min(100,Math.max(0,mPct)))+'%';
  }

  // Bonus card (continue to Maghrib) stays based on today's log
  const bc=document.getElementById('bonus-card');
  if(bc){
    if(log.targetReached && !log.maghribReached && todayTarget.type!=='maghrib'){
      bc.classList.remove('hidden');
      document.getElementById('bonus-timer').textContent=fmtTime(maghribT-now);
    } else {
...
function confirmAte(reason){
  ...
  // حالة النسيان: لا تخصم نقاط ولا تنهي الصيام — فقط سجّلها في اللوج
  if(reason==='forgot'){
    log.forgotCount=(log.forgotCount||0)+1;
    if(!log.notes) log.notes=[];
    log.notes.unshift({type:'forgot',time:Date.now(),text:'😅 أكل/شرب نسياناً — لا يؤثر على الصيام'});
    addActivityLog(p,'😅 نسي وأكل/شرب (لا يؤثر)',0,'approved');
    save();closeModal('m-ate');renderHome();toast('😅 لا بأس! هذا نسيان ولن يؤثر — كمل صيامك 💛');
    return;
  }
  ...
  const effects={
    ...
    forgot:{pts:0,penalty:false,msg:'لا بأس! الناسي معذور 😊'},
...
function renderQuizPath(p){
  ...
  // Build day states — STRICT sequential unlocking:
  // اليوم 2 لا يفتح إلا بعد إنهاء اليوم 1 ... وهكذا
  const DS=[];
  const doneByDay=(d)=>{
    const td=STATE.timetable.find(t=>t.ramadanDay===d);
    const dk=td?td.dateISO:null;
    const lg=dk&&p.logs&&p.logs[dk]?p.logs[dk]:null;
    return !!(lg&&lg.quizDone);
  };
  let firstIncomplete=1;
  for(let d=1;d<=Math.min(TOTAL,todayDay);d++){
    if(!doneByDay(d)){firstIncomplete=d;break;}
    if(d===todayDay) firstIncomplete=todayDay+1;
  }
  const activeDay=(firstIncomplete<=todayDay)?firstIncomplete:todayDay;
  ...
  for(let d=1;d<=TOTAL;d++){
    ...
    let state='locked';
    if(d<=todayDay){
      if(doneByDay(d)) state='done';
      else if(d===activeDay) state=(d===todayDay)?'today':'next';
      else state='locked';
    }
    DS.push({day:d,state,dateISO:dk,isEarliest:(d===activeDay && !doneByDay(d))});
  }
  ...
  const nodeStyle={
    done:   {ring:'#1a7a3a',bg:'linear-gradient(135deg,#27ae60,#1e8449)',icon:'✅',numColor:'#fff',shadow:'0 4px 15px rgba(39,174,96,.6)',border:'3px solid #1a7a3a',disabled:false,label:''},
    next:   {ring:'#e65c00',bg:'linear-gradient(135deg,#ff6b35,#e65c00)',icon:'▶️',numColor:'#fff',shadow:'0 0 0 5px rgba(255,107,53,.4),0 4px 20px rgba(255,107,53,.7)',anim:'animation:pulse-today 1.5s ease-in-out infinite',border:'3px solid #e65c00',disabled:false,label:'ابدأ هنا!'},
    ...
    today:  {ring:'#b7770d',bg:'linear-gradient(135deg,#f39c12,#d68910)',icon:'⭐',numColor:'#fff',shadow:'0 0 0 5px rgba(243,156,18,.35),0 4px 20px rgba(243,156,18,.7)',anim:'animation:pulse-today 1.8s ease-in-out infinite',border:'3px solid #b7770d',disabled:false,label:'اليوم'},
  ...
  // Clean header text (no extra title inside the map)
  const hintText='جاوب الأسئلة كل يوم واكسب نقاط وجواهر';
  let html=`
  <style>
    .adventure-map{border-radius:20px;overflow:hidden;position:relative;box-shadow:0 8px 32px rgba(0,0,0,.4)}
    /* More top padding so milestones (نصف شهر/ثلاثة أرباع/الختام) don't get clipped */
    .zone-row{position:relative;padding:30px 12px 18px;overflow:hidden;min-height:125px;display:flex;flex-direction:column;justify-content:center}
    /* Clear zone names on all backgrounds */
    .zone-label{position:absolute;top:8px;right:10px;font-size:0.66rem;font-weight:900;opacity:0.95;color:#fff;
      background:rgba(0,0,0,.35);border:1px solid rgba(255,255,255,.18);padding:4px 10px;border-radius:999px;
      text-shadow:0 1px 3px rgba(0,0,0,.55);backdrop-filter:blur(6px)}
    ...
    .milestone-flag{position:absolute;top:-14px;left:50%;transform:translateX(-50%);white-space:nowrap;font-size:0.58rem;font-weight:900;background:rgba(0,0,0,.55);color:#ffd700;padding:3px 7px;border-radius:8px;border:1px solid rgba(255,215,0,.4);backdrop-filter:blur(4px)}
    .explorer-bubble{position:absolute;top:-18px;left:50%;transform:translateX(-50%);white-space:nowrap;font-size:1.25rem;animation:bounce-exp 1s ease-in-out infinite}
  ...
  for(let row=0;row<totalRows;row++){
    ...
    display.forEach((ds,i)=>{
      ...
      html+=`<div class="map-node-wrap" style="${isMilestone?'margin-top:4px':''}">
        ...
        ${isToday||isNext?`<div class="explorer-bubble">${isNext?'👆':'🧒'}</div>`:''}
      ...
      if(!isLast){
        ...
        const connStyle=fromDone
          ...
          :fromToday
            ?`background:linear-gradient(90deg,#f39c12,rgba(255,255,255,.3))`
    ...
    if(row<totalRows-1){
      ...
      const vDone=lastLogical.state==='done'||lastLogical.state==='next';
  ...
  html+=`<div class="map-legend">
    <div class="map-legend-item"><div class="map-legend-dot" style="background:#27ae60"></div>مكتمل ✅</div>
    <div class="map-legend-item"><div class="map-legend-dot" style="background:#ff6b35"></div>اليوم التالي ▶️</div>
    <div class="map-legend-item"><div class="map-legend-dot" style="background:#f39c12"></div>اليوم ⭐</div>
    <div class="map-legend-item"><div class="map-legend-dot" style="background:rgba(255,255,255,.15);border:1.5px solid rgba(255,255,255,.3)"></div>مغلق 🔒</div>
  </div>`;
...
function renderCertificate(){
  ...
  // لا نعرض عدد الأيام/المتتالية في الشهادة — فقط عبارة الإتمام + مجموع النقاط
  const cd=document.getElementById('cert-days');
  if(cd) cd.textContent='صوم شهر رمضان المبارك 1447 هـ';
  document.getElementById('cert-pts').textContent=p.points||0;
  const cs=document.getElementById('cert-streak');
  if(cs){
    const blk=cs.parentElement;
...
function addReward(){
  ...
  const desc=document.getElementById('rew-desc')?.value.trim();
  const childId=csel?.value;
  if(!childId||!desc){toast('⚠️ اختر الطفل واكتب الهدية');return}
  const p=STATE.profiles.find(x=>x.id===childId);if(!p)return;
  if(!STATE.rewards)STATE.rewards=[];
  STATE.rewards.push({
    id: crypto?.randomUUID ? crypto.randomUUID() : ('rw_'+Date.now()+'_'+Math.floor(Math.random()*1e6)),
    childId, childName:p.name, childAv:p.avatar,
    text: desc,
...
function renderQuizQ(){
  ...
  const diffLabel={easy:'سهل 🟢',medium:'متوسط 🟡',hard:'صعب 🔴'};const dl=diffLabel[q.diff||(q.p<=10?'easy':q.p<=20?'medium':'hard')]||'';
  ...
  document.getElementById('quiz-container').innerHTML=`
    <div style="text-align:center;padding:16px 0 8px">
      ...
      <div class="card" style="margin-bottom:12px">
        ...
        <div style="background:rgba(255,255,255,.1);border-radius:8px;height:10px;overflow:hidden;margin-bottom:8px">
          <div style="height:100%;width:${pct}%;background:linear-gradient(90deg,#27ae60,#f0b429);border-radius:8px;transition:width .5s"></div>
        </div>
        <div style="display:flex;justify-content:space-between">
          <span style="font-weight:700">🎯 النقاط</span>
          <span style="font-weight:900;color:#f0b429">+${pts} نقطة</span>
        </div>
      </div>
      <div style="font-size:0.9rem;font-weight:700;color:var(--text);margin-bottom:16px;padding:8px 12px;background:rgba(240,180,41,0.1);border-radius:10px">${msg}</div>
